...
def test_home_favourite_spread_is_negative_and_above_half():
    ...
    assert prob_from_spread(-2.5) == pytest.approx(0.5735, abs=1e-3)
    assert prob_from_spread(3.5) == pytest.approx(0.3985, abs=1e-3)
...
def test_moneyline_devig_sums_to_one_and_removes_the_overround():
    # DEN @ KC, Week 1 2026: -192 / +160, a 4.3% vig.
    p = prob_from_moneyline(-192, 160)
    assert p == pytest.approx(0.572, abs=1e-3)
    assert p + prob_from_moneyline(160, -192) == pytest.approx(1.0)
...
def test_to_prob_prefers_moneyline_over_spread():
    o = _o("book", spread=-7.0, ml_home=-192, ml_away=160)
    assert to_prob(o) == pytest.approx(prob_from_moneyline(-192, 160))
...
def test_market_prob_averages_book_and_kalshi():
    book = _o("book", ml_home=-192, ml_away=160)
...
def test_espn_carries_raw_quoted_values_and_no_probability(espn_payload):
    ...
    assert kc.ml_home == pytest.approx(-192)
    assert kc.ml_away == pytest.approx(160)
...
def _snap(source, fetched_at, season=2026, week=1, spread=-2.5):
    return {"season": season, "week": week, "source": source,
            ...
            "games": [{"home": "KC", "away": "DEN", "spread": spread,
                       "total": 43.5, "ml_home": -192, "ml_away": 160,
...
def test_add_and_read_back_a_week(store):
    ...
    assert rows[0]["games"][0]["ml_home"] == -192
...
def test_export_and_import_carry_the_odds_log(tmp_path):
    ...
    assert len(got) == 1
    assert got[0]["id"] == rid
    assert got[0]["games"][0]["ml_away"] == 160
```
...
def test_snapshot_is_raw_and_carries_its_coordinates():
    ...
    assert doc["games"] == [{"home": "KC", "away": "DEN", "spread": None,
                             "total": None, "ml_home": -192, "ml_away": 160,
                             "yes_home": None, "yes_away": None, "p_home": None}]
...
def test_market_probs_keys_on_the_home_away_pair():
    rows = [{"source": "book", "games": [
        {"home": "KC", "away": "DEN", "spread": None, "total": None,
         "ml_home": -192, "ml_away": 160, "yes_home": None, "yes_away": None,
         "p_home": None}]}]
...
export interface WeekPlayer {
  ...
  chalk: number; dist: number[]; actual: number | null;
}
...
where `ifHome = base[name] + (1 - g.p_used) * swing` and `ifAway = ifHome - swing`, with `base` from the live projection's `pwin` per player.
...
Owns the week `<select>` (defaulting to the current week and offering every week in `store.list_weeks()` order), fetches via `getWeek`, polls every 60s while `state === 'live'`, and renders the two cards. Follow the polling and cleanup pattern in `Standings.tsx:27-37` exactly — `let alive = true`, a `setTimeout` chain, and clearing both on unmount.
...
def _at(ts, spread):
    return {"season": 2026, "week": 1, "source": "book", "fetched_at": ts,
            "games": [{"home": "KC", "away": "DEN", "spread": spread, "total": 43.5,
                       "ml_home": -192, "ml_away": 160, "yes_home": None,
                       "yes_away": None, "p_home": None}]}
...
- Modify: `scripts/cr-schedule.sh` — the Cloud Run equivalent
...
- [ ] **Step 3: Add the Cloud Run job**

In `scripts/cr-schedule.sh`, add a fifth scheduler job calling `/internal/refresh-odds` hourly at `20 * * * *`, following the shape of the existing four. Cloud Run is a paused rollback host; this is a cheap consistency, not a live path.
