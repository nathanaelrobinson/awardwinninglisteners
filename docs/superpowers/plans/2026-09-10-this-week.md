...
The swing axis is captioned with `<TeamLogo>` for away on the left and home on the right. A player's bar grows toward the team they need — positive swing means the home team — with length proportional to the largest absolute swing on that row. Omit any player whose `|swing| < 0.002`. The row's `title` reads:
