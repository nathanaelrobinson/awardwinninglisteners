...
export default function VoiceLegend(
  { voices, colors, refName, marketName }: {
    voices: string[]; colors: Record<string, string>; refName?: string; marketName?: string;
  },
) {
  return (
    <div className="viz-legend">
      {voices.map((v) => (
        <span key={v} className="viz-leg">
          ...
          {sourceLabel(v)}{colors[v] === OVERFLOW_COLOR ? ' (no colour slot left)' : ''}
        </span>
      ))}
      {refName != null && (
      ...
      {marketName != null && (
        <span className="viz-leg">
          <span className="viz-swatch viz-swatch-sq" />
          {marketName}
        </span>
      )}
    </div>
